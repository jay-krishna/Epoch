...
- **`<body>`** — header (brand with the timer/stopwatch mode toggle stacked below it, left-aligned; theme/mic/keep-awake/log buttons on the right), the timer face (SVG progress ring + hh/mm/ss inputs, editable in timer mode), footer controls, and a collapsible voice-log console.
...
- **Timer vs. stopwatch mode** (user-facing terms) is tracked internally as the `mode` variable (`"countdown"` | `"countup"`), mirrored on `<body data-mode="...">` for CSS (icon swap on `#modeBtn` — hourglass for timer, stopwatch glyph for stopwatch — driven the same way as the light/dark icon swap; `#modeBtn` also gets an always-on accent border/background so it reads as a primary control, not just another header icon). `setMode()` guards against switching while `state !== "idle"`; `#modeBtn.disabled` is kept in sync with app state via the `setAppState()` helper so the toggle is only usable when idle. The toggle button sits directly below the brand in `.left-group` (a column, left-aligned, with generous gap), separate from the right-side action icons. `setMode()` also raises a toast announcing the new mode (`showToast(...)`), the same feedback pattern used by `toggleWake()` for `#wakeBtn`.
  - Countdown (timer) keeps its original `endTime`/`totalMs`/`leftMs` fields and ring depletion.
  - Count-up (stopwatch) tracks `startTime` (running) / `elapsedMs` (paused) and counts up from 00:00:00 with no fixed total; the `hh/mm/ss` inputs are forced read-only in this mode (`setReadonly(mode === "countup")`), and the progress ring is simply set fully lit once on start/resume rather than animated per tick, since there's no "remaining fraction" to represent. Every `beepIntervalSec` elapsed seconds `tick()` calls `playBeep()` (a sharp double square-wave tone, tuned to be more noticeable than the countdown tick) as a cadence cue.
  - **Beep interval** is `beepIntervalSec` (default `10`), changeable only while in stopwatch mode via `#intervalRow` (hidden entirely in timer mode by CSS: `body:not([data-mode="countup"]) .interval-row{display:none}`). `#intervalBtn` shows the current value and toggles `#intervalDrawer` open/closed (`setIntervalDrawer()`); the drawer is a `.pill-btn` row per option (`5s/10s/30s/60s`) that expands rightward via a `max-width`/`opacity` transition (`.interval-drawer.open`), matching the available screen space since the control is pinned to the left edge. `setBeepInterval()` updates the label and the `.active` state on the matching option pill. A document-level click listener closes the drawer when clicking outside it.
  - `tick()`, `start()`, `pause()`, `resume()`, and `reset()` branch on `mode` for the pieces that differ; `finish()` (auto-reset on completion) only ever fires for countdown, since count-up has no end.
  - Mode switching works both from the UI button and by voice (`"Epoch switch timer"` / `"Epoch switch stopwatch"`), handled at the top of `handleCommand()` before the other command checks; both paths are gated on `state === "idle"` and funnel through `setMode()`, so the toast is shown consistently regardless of trigger. The voice parser also has small mode-aware guards elsewhere so existing commands stay correct — `"start"` in stopwatch mode starts immediately from zero (skips duration parsing), and `"set ..."` is rejected in stopwatch mode (no fixed duration to set).
